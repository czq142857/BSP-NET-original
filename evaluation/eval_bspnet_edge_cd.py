...
gt_point_dir = "/local-scratch/zhiqinc/zBSP/sample_point_edge_4096/"
bspnet_point_dir = "./test_edge/"
num_of_points = 4096
...
out_per_obj = open("result_ECD_per_obj.txt","w")
out_per_cat = open("result_ECD_per_category.txt","w")
...
with tf.Session(config=run_config) as sess:
    ...
    for idx in range(len(eval_list)):
        ...
        if len(gt_pc_vertices)==0:
            print(idx, -1, -1)
            out_per_obj.write( str(-1)+' '+str(-1)+'\n' )
            continue

        #read bspnet
        bspnet_ply_dir = bspnet_point_dir+str(idx)+'.ply'
        if len(bspnet_pc_vertices)==0:
            bspnet_pc_vertices = np.zeros([1,3],np.float32)
            bspnet_pc_normals = np.zeros([1,3],np.float32)
